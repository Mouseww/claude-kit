...
**Default to delegation.** Route non-trivial work to a subagent rather than reading and editing inline. The main thread orchestrates and decides; subagents return compact results — conclusions, `file:line` references, a change summary — never raw file contents. Reach for it early, at every stage, not just implementation. Exceptions at the end are real, but they are exceptions.

Delegation serves two goals (most work triggers only one):
...
2. **Keep typing off the expensive model.** Writing code costs model time on whatever tier the main thread runs. That cost never shows in the transcript, which is why it gets forgotten.
...
Vague request in, structured spec out -> `requirements-analyst`. Proposal or plan in, verdict or recommendation out -> `deepthink`. When unsure, prefer the cheaper agent; it will escalate if needed.
...
Delegation is not free: the subagent re-pays its system prompt plus your brief, and the main thread pays for the summary. Skip it for a single trivial edit, tight interactive back-and-forth, edits needing fresh judgement each time, creating substantial new content from scratch, or when reading one or two files is cheaper than the brief. Full policy: the `dev-agents` skill.
